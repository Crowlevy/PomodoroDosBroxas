...
// fetch configuraçãoes estipuladas pelo usuário
try {
    $stmt = $pdo->prepare("SELECT work_duration, break_duration FROM settings WHERE user_id = ?");
    $stmt->execute([$user_id]);
    $settings = $stmt->fetch(PDO::FETCH_ASSOC);
    
    // usando valores 'normais' caso não ocorra a mudança
    if (!$settings) {
        $settings = [
            'work_duration' => 25,
            'break_duration' => 5
...
// verificação de quantos pomodoros faltam
try {
    $stmt = $pdo->prepare("
...
<body>
    <div class="timer-container">
        ...
        <div class="phase-indicator" id="phase">Hora de broxar</div>
        
        <div class="break-message" id="breakMessage">
            Descansa aí irmão, você não é o David Goggins, seu inútil
        </div>

        <div class="timer-controls">
            <button class="start-btn" id="startBtn">Começar</button>
            <button class="pause-btn" id="pauseBtn" disabled>Pausar</button>
            <button class="reset-btn" id="resetBtn">Resetar</button>
        </div>
        
        <button class="resume-btn" id="resumeBtn" style="display: none;">Mostrar tarefa</button>
        <button class="skip-btn" id="skipBtn">Pular timer</button>
        <a href="dashboard.php" class="back-btn">Voltar para o dashboard</a>
    </div>

    <div class="overlay" id="overlay"></div>
    <div class="confirmation-dialog" id="skipDialog">
        <p>Você tem certeza que deseja deletar essa tarefa? PENSE BEM</p>
        <div class="dialog-buttons">
            <button onclick="confirmSkip()">Sim, sou fraco</button>
            <button onclick="closeSkipDialog()">Cancelar</button>
        </div>
    </div>
    ...
    <script>
        ...
        function completePomodoro() {
            ...
            .then(data => {
                ...
                if (data.is_complete) {
                    // lógica se caso o usuário completar todos os pomodoros, é redirecionado até o dashboard automaticamente
